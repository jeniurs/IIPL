...
English to German Transformers Translation Code
"""
...
class TranslateDataset(Dataset):
    def __init__(self, source_data=None, target_data=None, source_max_seq_len=256, target_max_seq_len=256, phase="train"):
        ...
        srce_file_path = "eng_text_file.txt"
        targ_file_path= "german_text_file.txt"
        ...
        #토크나이징
        source= spm.SentencePieceTrainer.train('--input=eng_text_file.txt  --model_prefix=wmt2016en --vocab_size=16000 --model_type=bpe --max_sentence_length=9999')
        ...
        #source_tokens = sp.EncodeAsPieces(source_data)

        #preprocessing
        source_array2=[]
        for itemss in source_array:
          result=preprocess_seq(itemss)
          result=result.split('▁')
          #for item2 in result:
            #item2=item2.split('')

          source_array2.append(result)
        #print(source_array2[1:4])
        ...
        with open("wmt2016en.vocab", "r", encoding="utf-8") as f:
            for line in f:
              word, index = line.strip().split('\t')  # 탭 문자로 분리
              self.vocab_dict[word] = int(index)
              self.new_dict = {key:abs(value) for key, value in self.vocab_dict.items()}
              self.new_dict[0] = '<PAD>'

            #print(self.new_dict)
        try:
         source_enc=[[self.new_dict[value] for value in eng] for eng in tqdm(source_array2)]
        except KeyError:
        ...
        #en_vocab=build_vocab('wmt2016en.vocab', sp_tokens_generator)


        #sp= spm.SentencePieceProcessor()
        #idx= sp.encode(source)
        #print(type(idx))
        #if self.phase == "train":
            #target=spm.SentencePieceTrainer.Train('--input=german_text_file.txt  --model_prefix=wmt2016de --vocab_size=16000 --model_type=bpe --max_sentence_length=9999')
        ...
        #<pad>으로 채워주자
        features = pad_features(source_enc, pad_id=0, seq_length=seq_length)
        print(features)

        assert len(features) == len(source_enc)
...
vocab={'a':2, 'b':4, 'c': 5}
review='aa bb cc'
w2int= [[vocab[value] for value in r.split()] for r in review]
for i in w2int:
  print(i)
